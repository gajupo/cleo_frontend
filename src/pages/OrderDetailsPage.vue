<template>
  <div class="container mt-4">
    ...
    <!-- Error Message for Minimum Spend -->
    <div v-if="minSpendError" class="alert alert-warning mt-3">
      {{ minSpendError }}
    </div>
...
export default {
  data() {
    return {
      ...
      loading: false,
      error: null,
      minSpendError: null, // Error message for minimum spend
    };
  },
  computed: {
    cartItems() {
  ...
  methods: {
    ...
    async placeOrder() {
      if (this.totalAmount < 150) {
        this.minSpendError = 'El pedido mínimo es de $150. Agrega más productos.';
        return;
      }
      ...
      this.loading = true;
      this.error = null;
      this.minSpendError = null; // Clear min spend error on valid order
      try {
        const payload = {
